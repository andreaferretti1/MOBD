function [W, b] = momentum_gradient_method(alpha, beta, train_X, train_Y, eval_X, eval_Y, neural_network, minibatch_size, max_epochs_num, regularization_parameter, isValidation)
...
for epoch = 1:max_epochs_num
    ...
    for idx = 1:length(minibatches)
        ...
        [grad_W, grad_b] = backpropagation_with_regularization(minibatch_data, train_Y(minibatches{idx}, :), neural_network, regularization_parameter, Y_predicted, a, z);
        ...
        train_loss(iteration) = mean(binary_cross_entropy(Y_predicted, train_Y), 'all');
        ...
        if(mod(iteration, eval_frequency) == 0)
            ...
            eval_loss(index) = mean(binary_cross_entropy(Y_predicted, eval_Y));
